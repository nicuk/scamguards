...
function ResultsContent({ sponsorSlot }: { sponsorSlot?: ReactNode }) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Rendered only alongside a finished result, so it never appears under
          the loading state and then gets shoved down when the result arrives */}
      {sponsorSlot && <div className="mt-10">{sponsorSlot}</div>}
    </div>
  );
}

export function ResultsView({ sponsorSlot }: { sponsorSlot?: ReactNode }) {
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      >
        <ResultsContent sponsorSlot={sponsorSlot} />
      </Suspense>
    </div>
  );
}
